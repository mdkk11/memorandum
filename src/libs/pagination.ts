export function getPaginationSrc({ page = 0, take = 9 }: { page?: number; take?: number }) {
...
export function generatePagination(current: number, max: number) {
  ...
  const prev = current === 1 ? null : String(current - 1);
  const next = current === max ? null : String(current + 1);
...
export function getPagination({ take, skip, currentPage, hitCount }: { take: number; skip: number; currentPage: number; hitCount: number }) {
