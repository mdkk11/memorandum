...
import { CreateDocumentServerSchema, DocumentSchema } from '@/schemas/document';
import { createDocument } from '@/services/documents/createDocument';

type State = typeof DocumentSchema;

export const createDocumentAction = async (prevState: FormState<State>, formData: FormData): Promise<FormState<State>> => {
  ...
  try {
    const payload = validateFormData(formData, CreateDocumentServerSchema);
